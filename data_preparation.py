...
def prepare_data(df):
    ...
    final_cols = ['Vol_name', 'Sta_yr', 'Sta_mo', 'Sta_dy', 
    ...
    df_prepared = pd.DataFrame(df, columns=['Vol_name', 'Sta_yr', 'Erup_dur', 'VEI'])
    return df_prepared

if __name__ == '__main__':
    new_original_file = "New_GVP_Eruption_Results.xlsx"
    df_original = pd.read_excel(new_original_file, sheet_name='Eruption List', skiprows=1)
